...
users_bp = Blueprint('users', __name__) #dp blueprint
...
@jwt_required() # un depurador, es funcion verifica la peticion si enviando token
def get_all_users():
    try:
...
def update_user(user_id):
    try:
        user_data = request.get_json()
        # buscar al usuario  ver si existe en base de datos
        user = User.query.get(user_id)
